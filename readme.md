...
if (!function_exists('wp_notify_moderator')) :
	...
	function wp_notify_moderator($comment_id)
	{
		...
		foreach ($emails as $email) {
			...
			$res_id = getCourseIdByLessonId($post_id);
			if ($res_id) {
				$course_id = $res_id;
				$lesson_id = $post_id;
			} else {
			...
			$email_content = aqskillCommentHtml($email, $course_id, $lesson_id, $message);
			wp_mail($email, wp_specialchars_decode($subject), $email_content, $message_headers);
		}

		//reset email content type to default
